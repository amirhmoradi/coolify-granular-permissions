    ...
    @if($sources->count() > 0)
        <div class="flex flex-col gap-4 mt-4">
            @foreach($sources as $source)
                <div class="p-4 rounded border border-neutral-200 dark:border-coolgray-300 bg-white dark:bg-coolgray-100 {{ !$source->enabled ? 'opacity-60' : '' }}">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                <h4 class="truncate">{{ $source->name }}</h4>
                                @if($source->last_sync_status === 'success')
                                    <span class="text-xs px-1.5 py-0.5 rounded bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400">synced</span>
                                @elseif($source->last_sync_status === 'failed')
                                    <span class="text-xs px-1.5 py-0.5 rounded bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400">failed</span>
                                @elseif($source->last_sync_status === 'syncing')
                                    <span class="text-xs px-1.5 py-0.5 rounded bg-yellow-100 text-yellow-700 dark:bg-yellow-500/20 dark:text-yellow-400">syncing...</span>
                                @endif
                                @if(!$source->enabled)
                            ...
                            <div class="text-sm text-neutral-600 dark:text-neutral-500 truncate mt-1">
                                {{ $source->repository_url }}
                            </div>
                            <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-neutral-600 dark:text-neutral-500 mt-1">
                            ...
                            @if($source->last_sync_status === 'failed' && $source->last_sync_error)
                                <div class="text-xs text-red-700 dark:text-red-400 mt-2 p-2 rounded bg-red-100 dark:bg-red-500/10">
                    ...
                    @if($source->template_count > 0)
                        <div class="mt-2">
                            <button
                                ...
                                class="text-xs text-neutral-600 hover:text-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-200 transition-colors"
                            >
                            ...
                            @if($expandedSourceUuid === $source->uuid)
                                <div class="mt-2 p-3 rounded bg-neutral-100 dark:bg-black/30">
                                    <div class="grid grid-cols-1 gap-1 text-xs md:grid-cols-2 lg:grid-cols-3">
                                        @forelse($this->expandedTemplates as $key => $template)
                                            <div class="flex items-center gap-2 p-1 rounded hover:bg-black/5 dark:hover:bg-white/5">
                                                @php
                                                ...
                                                @else
                                                    <div class="w-5 h-5 rounded bg-neutral-300 dark:bg-neutral-700 flex items-center justify-center text-xs">
                                                        {{ strtoupper(substr($key, 0, 1)) }}
                                                    </div>
                                                @endif
                                                <div class="truncate">
                                                    <span class="text-neutral-800 dark:text-neutral-200">{{ str($key)->headline() }}</span>
                                                    @if($slogan = data_get($template, 'slogan'))
                                                        <span class="text-neutral-600 dark:text-neutral-500 ml-1">— {{ Str::limit($slogan, 60) }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        @empty
                                            <div class="text-neutral-600 dark:text-neutral-500 col-span-full">No templates cached. Try syncing.</div>
                                        @endforelse
                                    </div>
                                </div>
                            @endif
