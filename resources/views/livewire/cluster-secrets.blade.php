<div>
    <div class="flex items-center justify-between pb-4">
        <div>
            <h3 class="font-semibold text-neutral-900 dark:text-white">Swarm Secrets</h3>
            <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-0.5">Docker secrets are encrypted at rest and only available to services granted explicit access.</p>
        </div>
    ...
    @if ($showCreateForm)
        <div class="p-4 mb-4 rounded-lg bg-white dark:bg-coolgray-200 border border-neutral-200 dark:border-coolgray-300">
            <h4 class="font-medium text-neutral-900 dark:text-white text-sm pb-3">Create New Secret</h4>
            <div class="space-y-3">
                ...
                <div>
                    <label class="block text-sm font-medium text-neutral-600 dark:text-neutral-300 pb-1">Secret Value</label>
                    <textarea wire:model="newValue" rows="3"
                        class="w-full px-3 py-2 text-sm rounded bg-white dark:bg-coolgray-100 border border-neutral-200 dark:border-coolgray-300 text-neutral-900 dark:text-white font-mono focus:border-blue-500 focus:outline-none"
                        placeholder="Enter the secret value..." style="-webkit-text-security: disc;"></textarea>
                </div>
                <div class="flex items-center gap-2 p-2 rounded bg-yellow-100 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/20">
                    <svg class="w-4 h-4 text-yellow-600 dark:text-yellow-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <span class="text-xs text-yellow-700 dark:text-yellow-400">Secret values cannot be viewed after creation. Store them securely.</span>
                </div>
    ...
    @if (count($secrets) === 0)
        <div class="p-12 text-center rounded-lg bg-white dark:bg-coolgray-200 border border-neutral-200 dark:border-transparent">
            ...
            <p class="text-neutral-600 dark:text-neutral-400">No secrets in this cluster.</p>
            <p class="text-sm text-neutral-600 dark:text-neutral-500 mt-1">Secrets provide encrypted storage for sensitive data like passwords and API keys.</p>
        </div>
    @else
