...
GHCR_IMAGE="ghcr.io/amirhmoradi/coolify-enhanced:latest"
LOCAL_IMAGE_NAME="coolify-enhanced:local"
...
check_addon_installed() {
    if [ -f "$CUSTOM_COMPOSE" ] && grep -q "coolify-enhanced\|COOLIFY_ENHANCED" "$CUSTOM_COMPOSE" 2>/dev/null; then
        return 0
    fi
    return 1
}
...
# Coolify Enhanced - Auto-generated by install.sh
...
EOF
    ...
    if [ -f "$ENV_FILE" ]; then
        if grep -q "COOLIFY_ENHANCED" "$ENV_FILE"; then
            sed -i 's/COOLIFY_ENHANCED=.*/COOLIFY_ENHANCED=true/' "$ENV_FILE"
            info "Updated COOLIFY_ENHANCED=true in .env"
        else
            echo "COOLIFY_ENHANCED=true" >> "$ENV_FILE"
            info "Added COOLIFY_ENHANCED=true to .env"
        fi
    else
    ...
    if docker exec coolify php artisan package:discover 2>/dev/null | grep -q "coolify-enhanced"; then
    ...
    if docker exec coolify php artisan migrate:status 2>/dev/null | grep -q "project_user"; then
        ...
        info "Or run manually: docker exec coolify php artisan migrate --path=vendor/amirhmoradi/coolify-enhanced/database/migrations --force"
    fi
    ...
    echo "  - Set: COOLIFY_ENHANCED=true"
...
action_uninstall_addon() {
    ...
    if [ -f "$uninstall_script" ]; then
        ...
        echo "  2. Remove COOLIFY_ENHANCED from ${ENV_FILE}"
...
action_check_status() {
    ...
    if [ -f "$ENV_FILE" ]; then
        local env_val
        env_val=$(grep "COOLIFY_ENHANCED" "$ENV_FILE" 2>/dev/null || echo "")
        if [ -n "$env_val" ]; then
            success "Environment: ${env_val}"
        else
            info "COOLIFY_ENHANCED not set in .env"
        fi
    fi
...
show_banner() {
    echo ""
    echo -e "${CYAN}${BOLD}===============================================${NC}"
    echo -e "${CYAN}${BOLD}  Coolify Enhanced - Setup${NC}"
    echo -e "${CYAN}${BOLD}===============================================${NC}"
    echo ""
}
